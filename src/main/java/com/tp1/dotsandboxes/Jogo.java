...
public class Jogo extends Game {
    ...
    @Override
    public void create() {
        setScreen(new JanelaInicial(this));
        criaObjetosJogo();
    }
    ...
    private void criaPersonagens(){
        ...
        thrower = new PersonagemNaoMovel(texturaThrower, new TextureRegion(texturaThrower, 0, 0, 35, 58),
            800, 1400, new Vector2(0.5f, 2.5f), 6, 3, ataqueThrower);
        ...
        jamminger = new PersonagemNaoMovel(texturaJamminger, new TextureRegion(texturaJamminger, 0, 0, 28, 35),
            900, 1600, new Vector2(0.3f, 3.0f), 3, 2, ataqueJamminger);

        inimigos.add(penguin);
        // inimigos.add(thrower);
        // inimigos.add(jamminger);
    ...
    private void colisaoMegaManMapa(){
        ...
        for (Rectangle plataforma : mapa.getChaos()) {
            if(colisaoCorpos(corpoMegaman, plataforma)){
                float posBaseMegaMan = corpoMegaman.y;
                float topoMegaMan = corpoMegaman.y + corpoMegaman.height;
                
                if(topoMegaMan > plataforma.y && posBaseMegaMan < plataforma.y + plataforma.height) {
                    if(megaMan.getVelY() <= 0 && posBaseMegaMan <= plataforma.y + plataforma.height && 
                       topoMegaMan >= plataforma.y) {
                        megaMan.setNoAr(false);
                        megaMan.setNaPlataforma(true);
                        megaMan.setPosicao(megaMan.getPosX(), plataforma.y + plataforma.height);
                        megaMan.setVelY(0);
                        colidiu = true;
                        break;
                    }
        ...
        if(!colidiu && megaMan.isNaPlataforma()) {
            megaMan.setNaPlataforma(false);
    ...
    private void colisaoMegaManPersonagens(){
        if(gerenciadorColisoes.colisaoMegaManInimigos(megaMan, inimigos)){
            megaMan.setColidiuInimigo(true);
            megaMan.tomarDanoPorContato(10);
        }
